                                        ...
                                        if i.objectId==$('.publish').find('.title').attr('rel')
                                            ...
                                            if check
                                                if window.SITE.SITE_USER_LEVEL<CONST.SITE_USER_LEVEL.EDITOR
                                                    ...
                                                    AV.Cloud.run("PostInbox.publish",{site_id:SITE.ID,post_id:i.objectId,title:$('.publish .title').val(),brief:$('.publish .summary').val(),tag_list:tag},{success:(m)->
                                                        console.log m
                                            })
                                            else
                                                AV.Cloud.run("PostInbox.rm",{site_id:SITE.ID,post_id:i.objectId,title:$('.publish .title').val(),brief:$('.publish .summary').val(),tag_list:tag},{success:(m)->
                                                    console.log m
                                            })
                            }
                        }
